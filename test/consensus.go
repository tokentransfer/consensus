...
func NewConsensusService(n *consensus.Node) *ConsensusService {
	return &ConsensusService{n}
}
...
func (service *ConsensusService) VerifyTransaction(rootAccount libcore.Address, t libblock.Transaction) (bool, error) {
	...
	ss := entry.GetStorageService()
	ms := entry.GetMerkleService()
	...
	if rootAccount != nil {
		...
		info, err := entry.GetAccountInfo(fromAccount)
		if err != nil || info == nil {
...
func (service *ConsensusService) getAccountInfo(rootAccount libcore.Address, account libcore.Address, sequence uint64) (*block.AccountState, *block.AccountState, error) {
	...
	lastInfo, _ := entry.GetAccountInfo(account)
	if lastInfo != nil {
		s, err := block.CloneState(lastInfo)
		if err != nil {
			return nil, nil, err
		}
		h, err := block.HashState(s)
		if err != nil {
			return nil, nil, err
		}
...
func (service *ConsensusService) ProcessTransaction(rootAccount libcore.Address, t libblock.Transaction) (libblock.TransactionResult, []libblock.State, error) {
	config := service.n.GetConfig()
	entry, err := service.n.GetEntry(rootAccount)
	if err != nil {
		return 0, nil, err
	}
	...
	tx, ok := t.(*block.Transaction)
	if !ok {
		return 0, nil, util.ErrorOfInvalid("transaction", t.GetHash().String())
	}
	...
	if tx.Sequence != fromSequence {
		return 0, nil, util.ErrorOf("error", "sequence", fmt.Sprintf("%d != %d", tx.Sequence, fromSequence))
	}
	...
	if rootAccount != nil {
		if tx.Gas < gas {
			return 0, nil, util.ErrorOf("insufficient", "gas", fmt.Sprintf("%d < %d", tx.Gas, gas))
		}
	...
	for _, a := range stateList {
		...
		if entry.lastInfo == nil && entry.info == nil {
			return 0, nil, util.ErrorOfInvalid("account info", a)
		} else {
			if entry.lastInfo == nil {
				states = append(states, entry.info)
			} else {
				lastHash, err := block.HashState(entry.lastInfo)
				if err != nil {
					return 0, nil, err
				}
				hash, err := block.HashState(entry.info)
				if err != nil {
					return 0, nil, err
				}
	...
	return block.TrSUCCESS, states, nil
}
